...
export class MedicineListComponent implements OnInit {
  ...
  loadMedicines(): void {
    ...
    obs.subscribe({
      ...
      error: (err) => {
        this.errorMessage = err?.error?.message || 'Failed to load medicines.';
        this.loading = false;
      }
    });
  }
  ...
  showSuccess(msg: string): void { this.successMessage = msg; setTimeout(() => { this.successMessage = ''; }, 3000); }
  isExpired(expiryDate: string): boolean { return expiryDate ? new Date(expiryDate) < new Date() : false; }
  ...
  get lowStockCount() { return this.medicines.filter(m => m.stockQuantity < 10).length; }
  get expiredCount()  { return this.medicines.filter(m => this.isExpired(m.expiryDate || '')).length; }
  get totalValue()    { return this.medicines.reduce((s, m) => s + (Number(m.price||0) * Number(m.stockQuantity||0)), 0); }
}